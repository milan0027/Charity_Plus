...
export const addPost = (formData) => async (dispatch) => {
  ...
  if (image !== "") {
    try {
      ...
      const response = await instance.post(
        "https://api.cloudinary.com/v1_1/webbid/image/upload",
        form
      );
      ...
      array.push(
        response.data.url.replace(
          "upload/",
          "upload/w_800,h_600,c_pad,b_white/"
        )
      );
    } catch (err) {
      console.log(err);
  ...
  try {
    ...
    dispatch(
      setAlert(value === "event" ? "Event Created" : "Post Created", "success")
    );
  } catch (err) {
...
export const approveComment = (postId, commentId) => async (dispatch) => {
  ...
  try {
    await axios.put(
      `/api/posts/comment/approve/${postId}/${commentId}`,
      config
    );
